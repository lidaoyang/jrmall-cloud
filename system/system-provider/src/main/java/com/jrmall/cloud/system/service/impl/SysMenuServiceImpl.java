...
import com.jrmall.cloud.common.base.KeyValue;
...
import com.jrmall.cloud.system.enums.MenuTypeEnum;
import com.jrmall.cloud.common.enums.StatusEnum;
import com.jrmall.cloud.system.converter.MenuConverter;
import com.jrmall.cloud.system.mapper.SysMenuMapper;
...
public class SysMenuServiceImpl extends ServiceImpl<SysMenuMapper, SysMenu> implements SysMenuService {
    ...
    public List<RouteVO> getCurrentUserRoutes() {
        ...
        Set<String> roleCodes = RpcUtil.getRoles();
     ...
     * 根据RouteBO创建RouteVO
     */
    private RouteVO toRouteVo(SysMenu menu) {
        ...
        // 【菜单】是否开启页面缓存
        if (Objects.equals(MenuTypeEnum.MENU.getValue(), menu.getType())
    ...
    public boolean saveMenu(MenuForm menuForm) {
        ...
        if (Objects.equals(MenuTypeEnum.CATALOG.getValue(), menuForm.getType())) {  // 如果是目录
            String path = menuForm.getRoutePath();
            if (menuForm.getParentId() == 0 && !path.startsWith("/")) {
                menuForm.setRoutePath("/" + path); // 一级目录需以 / 开头
            }
            menuForm.setComponent("Layout");
        } else if (Objects.equals(MenuTypeEnum.EXTLINK.getValue(), menuForm.getType())) {
            // 外链菜单组件设置为 null
        ...
        // 新增类型为菜单时候 路由名称唯一
        if (Objects.equals(MenuTypeEnum.MENU.getValue(), menuForm.getType())) {
